...
bool Metronet::isConsistent() {
    ...
    for (unsigned int bleh = 0; bleh < sot.size(); bleh++) {
        bool yn = false;
        for (unsigned int treinie = 0; treinie < _trams.size(); treinie++) {             // check every train
...
TEST(Consistence, stationOneTrackNoTram){
    Metronet* net = new Metronet();
    Station* st1 = new Station("A");
    Station* st2 = new Station("B");
    ...
    st1->setVolgende(1, st2);
    st1->setVorige(2, st2);
    st1->setVolgende(2, st2);
    st1->setSporen(vec);

    st2->setVolgende(1, st1);
    st2->setVorige(1, st1);
    st2->setVorige(2, st2);
    st2->setVolgende(2, st2);
    st2->setSporen(vec);
